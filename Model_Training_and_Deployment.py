...
#x_train = x_train.register(workspace=workspace, name='Diabetes_X', description='Processed data for training', tags={'format':'CSV'}, create_new_version=True)
...
Dataset.Tabular.register_pandas_dataframe(dataframe=x_train, target=datastore, name="x_train")
Dataset.Tabular.register_pandas_dataframe(dataframe=x_train, target=datastore, name="y_train")
...
model = Model.register(workspace=workspace,
                       model_name='Diabetes_Classification2',                # Name of the registered model in your workspace.
                       model_path="outputs/Modelfile_for_OP/BestModel.pkl",  # Local file to upload and register as a model.
                       ...
                       tags={'class': 'yes/no', 'type': 'classification'})

"""create_new_model = False
registered_model_name = 'Diabetes_Classification1'
model_version = 2
...
#Load the model
model = joblib.load(model_path + '/BestModel.pkl')
...
"""
("Best run_step: " + maximum_score_runid)
